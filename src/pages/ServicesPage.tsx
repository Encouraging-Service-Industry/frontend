...
export default function ServicesPage({ onOpenProvider, preselectedService, preselectedLocation, onOpenVendorDetail }: Props) {
              ...
              <button key={p.id} onClick={() => onOpenProvider?.(p)} className="w-full text-left bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex items-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 mb-1">
                    {p.service} • ¥{p.price ?? '-'} • {(p.location || '').split(',')[0] || ''} • 
