...
export default function ProviderDetailPage({
  ...
  const providerStories = stories
    .filter(
      (story) =>
        story.name === provider.name ||
        story.name === `${provider.name} (Provider)`
    )
    .sort((a, b) => b.timestamp - a.timestamp); // Filter and sort provider's own stories

  const [activeTab, setActiveTab] = useState<
    "services" | "stories" | "reviews"
  >("services");

  const tabButtonClass = (tabName: "services" | "stories" | "reviews") =>
    `py-2 px-4 text-sm font-medium rounded-full transition-colors ${
      activeTab === tabName
  ...
  return (
    <div className="p-4 pt-6">
      ...
      <div className="flex items-center mb-4">
        <img
          src={provider.avatar}
          className="rounded-full mr-4 shadow-md w-20 h-20 object-cover"
          width={80}
          height={80}
          alt={provider.name}
        />
      ...
      <div className="flex justify-around border-b border-gray-200 mb-6">
        <button
          ...
          className={tabButtonClass("services")}
        >
          Service Details
        </button>
        <button
          onClick={() => setActiveTab("stories")}
          className={tabButtonClass("stories")}
        >
          Stories ({providerStories.length})
        </button>
        <button
          onClick={() => setActiveTab("reviews")}
          className={tabButtonClass("reviews")}
        >
          Reviews ({provider.clientReviews?.length || 0})
        </button>
      </div>
      ...
      {activeTab === "services" && (
        <>
          ...
          <div className="mb-6">
            ...
            <img
              src="https://placehold.co/400x200/f1f5f9/4f46e5?text=Service+Description"
      ...
      {activeTab === "stories" && (
        <div className="mb-6">
          <h3 className="font-semibold text-lg text-gray-800 mb-3">
            {provider.name}'s Stories
          </h3>
          <div className="space-y-4">
            {providerStories.length === 0 ? (
              <div className="text-gray-500">
                No stories posted by {provider.name} yet.
              </div>
            ) : (
              providerStories.map((s) => (
                ...
                >
                  ...
                  <p className="text-gray-600 mb-3">{s.content}</p>
                  {s.image && (
                    <img
                      src={s.image}
                      className="rounded-lg w-full mb-3"
      ...
      {activeTab === "reviews" && (
        <div className="mb-6">
          <h3 className="font-semibold text-lg text-gray-800 mb-3">
            Client Reviews ({provider.clientReviews?.length || 0})
          </h3>
          <div className="space-y-4">
